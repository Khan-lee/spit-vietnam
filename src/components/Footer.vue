<template>
  <footer class="bg-primary text-black py-16 px-6 md:px-12 border-t border-slate-800/50">
    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
      ...
      <div class="space-y-6">
        <div class="flex items-center gap-4 flex-wrap">
          ...
          <!-- ⚡ UPDATE: TÊN THƯƠNG HIỆU chỉ hiện khi Admin có cấu hình (bỏ chữ mặc định "Vietnam") -->
          <span
            v-if="config.siteName || config.brandName || config.site_name"
            class="font-black text-xl md:text-2xl uppercase tracking-tight text-slate-900"
          >
            {{ config.siteName || config.brandName || config.site_name }}
          </span>
        </div>
        ...
        <div class="flex items-center gap-4 pt-2">
          <!-- Facebook Icon -->
          <a :href="config.facebook || 'https://www.facebook.com/vnspit'" target="_blank" rel="noopener noreferrer" class="w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center hover:scale-110 transition-all group">
          ...
          <!-- YouTube Icon -->
          <a :href="config.youtube || 'https://www.youtube.com/channel/UCevP--LZsdjHn3EQwgesoFA'" target="_blank" rel="noopener noreferrer" class="w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center hover:scale-110 transition-all group">
      ...
      <div class="space-y-6">
        ...
        <ul class="space-y-3 text-sm font-bold text-black">
          <!-- ⚡ UPDATE: CHỈ hiển thị các bài Chính sách do Admin tạo (collection 'policies',  -->
          <!-- category = 'chinh-sach'). Đã BỎ danh sách cứng cũ trỏ /ho-tro/... (route không   -->
          <!-- tồn tại -> bấm ra trang trắng). Chưa load xong thì hiện khung xám (skeleton).    -->
          ...
          <template v-else>
            <li v-for="n in 4" :key="n" class="h-4 w-3/4 rounded bg-black/10 animate-pulse"></li>
          </template>
      ...
      <div class="space-y-6">
        ...
        <ul class="space-y-4 text-sm text-black">
          <!-- ⚡ UPDATE: từng dòng chỉ hiện khi Admin có nhập (bỏ địa chỉ không dấu / "1900 xxxx" mặc định) -->
          <li v-if="config[`address_${locale}`] || config.address" class="flex items-start gap-3 group">
            <span class="text-black">📍</span>
            <span class="hover:text-slate-700 transition-colors leading-relaxed whitespace-pre-line">
              {{ config[`address_${locale}`] || config.address }}
            </span>
          </li>
          <li v-if="config.hotline" class="flex items-center gap-3 font-bold text-black">
            <span class="text-black">📞</span>
    ...
    <div class="max-w-7xl mx-auto mt-16 pt-8 border-t border-slate-800/50 flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] font-black uppercase tracking-widest text-black">
      <p class="italic">© CÔNG TY TNHH DỤNG CỤ CÔNG NGHIỆP CHÍNH XÁC SÀI GÒN. All rights reserved.</p>
...
onMounted(() => {
  ...
  unsubPolicies = onSnapshot(collection(db, 'policies'), (snapshot) => {
    ...
    // ⚡ UPDATE: nếu đọc lỗi (VD Firestore Rules chặn khách chưa đăng nhập đọc 'policies')
    // thì vẫn thoát trạng thái skeleton, để trống 2 cột thay vì hiện link giả/link chết.
