...
const addToCart = (item) => {
  ...
  setTimeout(() => {
    ...
    // Lấy đúng tên sản phẩm theo ngôn ngữ để hiện toast
    const pName = item[`name_${locale.value}`] || item.name

    if (existingItem) {
      existingItem.quantity += 1
    } else {
      cart.push({ id: route.params.id, name: pName, price: item.price, image: item.image, quantity: 1 })
    }
    ...
    toastMessage.value = locale.value === 'vi' 
      ? `Đã thêm "${pName}" vào giỏ hàng!` 
      : `Added "${pName}" to cart successfully!`
  ...
  <div class="min-h-screen bg-slate-50 font-sans antialiased text-slate-900">
    <transition name="slide-fade">
      <div v-if="showToast" class="fixed top-24 right-6 md:right-10 z-100 max-w-sm bg-slate-900 text-white p-5 rounded-2xl shadow-2xl flex items-center gap-4 border border-slate-700 backdrop-blur-sm">
        ...
        <div class="grow">
          <p class="text-[10px] font-black uppercase text-blue-400 tracking-widest mb-1">{{ locale === 'vi' ? 'THÀNH CÔNG' : 'SUCCESS' }}</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16 items-start bg-white p-8 md:p-12 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-100/50">
        ...
        <div class="space-y-6 md:space-y-8">
          <div>
            ...
            <p class="text-slate-400 font-bold uppercase text-xs tracking-[0.3em] ml-1">
          ...
          <div class="flex items-center gap-4 mt-6">
            <div :class="[
              ...
              ● {{ product.stock > 0 ? (locale === 'vi' ? 'Còn hàng' : 'In Stock') : (locale === 'vi' ? 'Hết hàng' : 'Out of stock') }}
            </div>
            <div v-if="product.stock > 0" class="text-slate-400 text-[10px] font-bold uppercase tracking-widest">
              {{ locale === 'vi' ? 'Sẵn có trong kho:' : 'Available in stock:' }} <span class="text-slate-900">{{ product.stock }}</span>
            </div>
          </div>
          ...
          >
            ...
            <span class="relative z-10 tracking-[0.2em] flex items-center justify-center gap-2">
              <template v-if="product.stock > 0">
                {{ isAdding ? (locale === 'vi' ? 'ĐANG XỬ LÝ...' : 'PROCESSING...') : (locale === 'vi' ? 'THÊM VÀO GIỎ HÀNG 🛒' : 'ADD TO CART 🛒') }}
              </template>
              <template v-else>{{ locale === 'vi' ? 'SẢN PHẨM TẠM HẾT HÀNG' : 'PRODUCT OUT OF STOCK' }}</template>
            </span>
          </button>

          <div v-if="product[`description_${locale}`] || product.description" class="mt-10 pt-8 border-t border-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div class="flex items-center gap-3 mb-6">
              <span class="w-1.5 h-6 bg-red-600 rounded-full"></span>
              <h3 class="text-sm font-black uppercase tracking-[0.2em] text-slate-900 italic">
                {{ locale === 'vi' ? 'Thông số & Đặc điểm' : 'Specs & Features' }}
              </h3>
            </div>
            <div class="bg-slate-50/50 p-6 rounded-4xl border border-slate-100/80 backdrop-blur-sm">
              <p class="text-slate-600 text-sm leading-relaxed font-medium whitespace-pre-line tracking-tight">
